...
void ahcorz()
{
    ...
    rep(m)
    {
        ...
        adj[u + n].pb({v + n, w / 2}), adj[v + n].pb({u + n, w / 2});
    }
    rep(n) if (good[i])
    {
        adj[i].pb({n + i, 0});
    }
    vll dis1 = dijkstra<ll>(0, adj), dis2 = dijkstra<ll>(n - 1, adj);
    ll mn = inf<ll>;
    rep(n) chmin(mn, max(min(dis1[i], dis1[i + n]), min(dis2[i], dis2[i + n])));
    if (mn > inf<ll> / 2)
        print(-1);
    else
        print(mn);
